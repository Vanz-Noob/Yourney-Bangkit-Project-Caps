package com.bangkit.yourney.ui.main.favorite
...
import com.bangkit.yourney.ui.main.home.DestinationListAdapter
...
class FavoriteDestinationFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (activity != null) {
            listener = requireActivity() as HomeFragment.ToggleListener
            initDestinationData()
            with(binding) {
...
//                    if (s.isNotEmpty()) tietSearch.setText("")
    ...
    private fun getAllDestination() {
        viewState(isLoading = true, isError = false, isEmpty =  false, showView = false)
        val livedata = userViewModel.getFavorite(AuthPreference(requireContext()).getToken)
        livedata.observe(viewLifecycleOwner) { data ->
            if (data != null) {
                when (data) {
                    is Resource.Loading<*> -> {
        ...
        livedata.observe(viewLifecycleOwner) { data ->
            if (data != null) {
                when (data) {
                    ...
                    is Resource.Error<*> -> {
                        Log.d("TAG_LOADING", data.message.toString())
                        GlobalUtils.showToast(
                            data.message.toString(),requireContext()
                        )
                        livedata.removeObservers(viewLifecycleOwner)
                    }
                }
